          ...
          req.on('end', async () => {
            try {
              ...
              if (!apiKey || apiKey === 'YOUR_GROQ_API_KEY') {
                ...
                res.end(JSON.stringify({ error: 'GROQ_API_KEY não configurada. Adicione sua chave real no arquivo .env.local (https://console.groq.com/keys)' }));
                return;
              }

              const { prompt, type, pdfBase64 } = JSON.parse(body);
              ...
              if (!apiResponse.ok) {
                ...
                if (apiResponse.status === 429) {
                  message = "Limite de uso atingido. Aguarde um instante e tente novamente.";
                }
                res.writeHead(500, { 'Content-Type': 'application/json' });
              ...
              res.writeHead(500, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ error: error.message || 'Erro interno do servidor local.' }));
            }
          });
          return;
        }
